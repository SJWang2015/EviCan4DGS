...
class CameraData(object):
    def __init__(
        ...
        self.lidar_flows_maps = None
        self.lidar_flow_classes_maps = None
        self.lidar_points_maps = None
    ...
    def load_lidar_flows(
        self,
        lidar_flows_maps: List,
    ):
        self.lidar_flows_maps = lidar_flows_maps


    def load_lidar_flow_classes(
        self,
        lidar_flow_classes_maps: List,
    ):
        self.lidar_flow_classes_maps = lidar_flow_classes_maps  


    def load_lidar_points(
        self,
        lidar_points_maps: List,
    ):
        self.lidar_points_maps = lidar_points_maps

    # def load_liadr_to_worlds(
    #     self,
    #     liadr_to_worlds_maps: List,
    # ):
    #     self.liadr_to_worlds_maps = liadr_to_worlds_maps
    ...
    def to(self, device: torch.device):
        ...
        if self.lidar_flows_maps is not None:
            self.lidar_flows_maps = [item.to(device) for item in self.lidar_flows_maps] 
        if self.lidar_flow_classes_maps is not None:
            self.lidar_flow_classes_maps = [item.to(device) for item in self.lidar_flow_classes_maps] 
        if self.lidar_points_maps is not None:
            self.lidar_points_maps = [item.to(device) for item in self.lidar_points_maps] 
        # if self.liadr_to_worlds_maps is not None:
        #     self.liadr_to_worlds_maps = [item.to(device) for item in self.liadr_to_worlds_maps]
    ...
    def get_image(self, frame_idx: int) -> Dict[str, Tensor]:
        ...
        lidar_flows_maps = None
        lidar_flow_classes_maps = None
        if self.lidar_flows_maps is not None:
            lidar_flows_maps = self.lidar_flows_maps[frame_idx]
            lidar_flow_classes_maps = self.lidar_flow_classes_maps[frame_idx]

            # if self.downscale_factor != 1.0:
            #     lidar_depth_map = sparse_lidar_map_downsampler(lidar_depth_map, self.downscale_factor)
        lidar_points_maps = None
        if self.lidar_points_maps is not None:
            lidar_points_maps = self.lidar_points_maps[frame_idx]
            # lidar_to_worlds_maps = self.liadr_to_worlds_maps[frame_idx]
        ...
        # cam_pts_xyz = origins + viewdirs * direction_norm
        ...
        _image_infos = {
            ...
            "lidar_flows_map": lidar_flows_maps,
            "lidar_points_map": lidar_points_maps,
            "lidar_flow_classes_map": lidar_flow_classes_maps
            # "lidar_to_worlds_map": lidar_to_worlds_maps,
        }
...
class ScenePixelSource(abc.ABC):
    ...
    def propose_training_image(
        ...
        if random.random() < self.buffer_ratio and self.image_error_buffered:
            ...
            # indices_iter = iter(candidate_indices)
            # img_idx = next(indices_iter)  # 第一次是5
    ...
    def prepare_novel_view_render_data(self, dataset_type: str, traj: torch.Tensor, lidar_source: SceneLidarSource = None) -> list:
        """
        ...
        for i in range(len(traj)):
            ...
            # Lidar pints
            # normed_time = self.normalized_time[scaled_indices[i].round().long()]
                
            # get lidar depth on image plane
            closest_lidar_idx = lidar_source.find_closest_timestep(normed_time[i])
            ...
            lidar_cam_pts = lidar_points_xyz
            ...
            image_infos = {
                ...
                "lidar_points_map": lidar_cam_pts,
                "lidar_flows_map": flow,
                "lidar_flow_classes_map":flow_class

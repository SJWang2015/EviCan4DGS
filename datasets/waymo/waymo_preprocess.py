...
class WaymoProcessor(object):
    ...
    def convert_one(self, file_idx):
        ...
        for frame_idx, data in enumerate(
            ...
            # print(type(bytearray(data.numpy())))
    ...
    def save_depth_image(self, frame, file_idx, frame_idx):
        mask_dir = f"{self.save_dir}/{str(file_idx).zfill(3)}/depth/"
        if not os.path.exists(mask_dir):
            os.makedirs(mask_dir)
        ...
        if range_image_top_pose is None:
            # the camera only split doesn't contain lidar points.
            return
        depth_img_list = get_camera_depth_image(frame, range_images, range_image_top_pose=range_image_top_pose, camera_projections=camera_projections, ri_index=0,)
        for i, img in enumerate(depth_img_list):
            depth_image_numpy = img.numpy().squeeze() # 降维，从 [H, W, 1] 到 [H, W]
            # 归一化深度值到 0-255
            # 这里我们假设最大有效深度为 80 米
            # MAX_DEPTH = 80.0
            # # 将所有大于 MAX_DEPTH 的值设为 MAX_DEPTH
            # depth_image_numpy[depth_image_numpy > MAX_DEPTH] = MAX_DEPTH
            # # 线性归一化
            # normalized_depth = (depth_image_numpy / MAX_DEPTH) * 255.0

            # 转换为 uint8 类型
            dynamic_mask = np.clip((depth_image_numpy > 0.1) * 255, 0, 255).astype(np.uint8)
            dynamic_mask = Image.fromarray(dynamic_mask, "L")
            # depth_image_8bit = depth_image_numpy.astype(np.uint8)

            # 使用 Pillow 保存
            # img_8bit = Image.fromarray(dynamic_mask)
            dynamic_mask.save(mask_dir+f"{str(frame_idx).zfill(3)}_{i}.png")

            # print(f"{str(frame_idx).zfill(3)}_{i}.png")

            # tf.io.write_file(f"{str(frame_idx).zfill(3)}_{i}.png", img_8bit)
        # dynamic_mask = np.clip((dynamic_mask > 1.0) * 255, 0, 255).astype(np.uint8)
        # dynamic_mask = Image.fromarray(dynamic_mask, "L")
        # dynamic_mask_path = os.path.join(mask_dir, f"{str(frame_idx).zfill(3)}_{str(img.name - 1)}.png")
        # dynamic_mask.save(dynamic_mask_path)

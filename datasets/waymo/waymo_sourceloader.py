...
class WaymoLiDARSource(SceneLidarSource):
    ...
    def load_lidar(self):
        ...
        for t in trange(
            ...
            # pred_next = lidar_points + lidar_flows / 1.0
            ...
            #     if t > 0:
            #         next_t = t - 1
            #         next_lidar_info = np.memmap(
            #             self.lidar_filepaths[next_t],
            #             dtype=np.float32,
            #             mode="r",
            #         ).reshape(-1, 14)


            #         next_lidar_points = torch.from_numpy(next_lidar_info[:, 3:6]).float()
            #         next_lidar_flows = torch.from_numpy(next_lidar_info[:, 6:9]).float()
            #         next_lidar_points = (self.lidar_to_worlds[next_t][:3, :3] @ next_lidar_points.T
            #         + self.lidar_to_worlds[next_t][:3, 3:4]).T
            #         pred_next = lidar_points - lidar_flows / 10.0
            #         # pred_next = (self.lidar_to_worlds[t][:3, :3] @ pred_next.T
            #         # + self.lidar_to_worlds[t][:3, 3:4]).T
                    
            #         visualize_point_cloud([lidar_points, next_lidar_points, pred_next],[[0,1,0], [1,0,0], [0,0,1]])
    ...
    def get_lidar_rays(self, time_idx: int) -> Dict[str, Tensor]:
        ...
        flow_classes = self.flow_classes[self.timesteps == time_idx] 
        return {
            ...
            "lidar_flows": flows,
            "lidar_flow_classes": flow_classes
        }

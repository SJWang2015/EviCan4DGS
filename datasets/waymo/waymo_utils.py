        ...
        if (
            ...
            if laser.name == dataset_pb2.LaserName.TOP:
                ...
                range_image_top_pose.ParseFromString(
                   ...
                   range_image_top_pose_str_tensor.numpy()
                )
...
def get_camera_depth_image(frame,
                    range_images,
                    range_image_top_pose,
                    camera_projections,
                    ri_index=0,
    ...
    for c in calibrations:
        ...
        range_image_origins_cartesian, range_image_points_cartesian = extract_point_cloud_from_range_image(
            tf.expand_dims(range_image_tensor[..., 0], axis=0),
            extrinsic,
            tf.expand_dims(tf.convert_to_tensor(beam_inclinations), axis=0),
            pixel_pose=pixel_pose_local,
        ...
        # cc = frame.context.camera_calibrations
        for cc in frame.context.camera_calibrations:
            if c.name != cc.name:
                continue
            # print(cc.height)
            # print(range_image_points_cartesian.shape)
            # print(camera_projections[c.name][ri_index].shape)
            shape_proto = camera_projections[c.name][ri_index].shape
            shape_list = [d for d in shape_proto.dims]  # 转成 [64, 2650, 6]
            data = np.array(camera_projections[c.name][ri_index].data)        # flat array
            data = data.reshape(shape_list)                                   # reshape
            cam_proj = tf.convert_to_tensor(data, dtype=tf.int32)           # 转 Tensor
            cam_proj = tf.expand_dims(cam_proj, axis=0)
            # print(cam_proj.shape)
            # print(extrinsic.shape)
        
            depth_img = range_image_utils.build_camera_depth_image(range_image_points_cartesian,
                                    tf.cast(extrinsic, tf.float32),
                                    camera_projection = cam_proj,
                                    camera_image_size = [cc.height, cc.width],
                                    camera_name=c.name)
            # print(depth_img.shape)
            depth_img_list.append(depth_img[0])

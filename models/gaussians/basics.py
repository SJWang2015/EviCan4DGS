...
def dup_in_optim(optimizer, dup_mask, param_dict, n=2):
    ...
    for group_idx, group in enumerate(optimizer.param_groups):
        ...
        if name in param_dict.keys():
            ...
            if dup_mask ==  torch.bool:
                comp_avg_tensor = torch.zeros_like(param_state["exp_avg"][dup_mask.squeeze()]).repeat(*repeat_dims)
                comp_avg_sq_tensor = torch.zeros_like(param_state["exp_avg_sq"][dup_mask.squeeze()]).repeat(*repeat_dims)
            elif dup_mask.dtype == torch.bool:
                old_shape = param_state["exp_avg"].shape
                new_first_dim = dup_mask.shape[0]
                comp_avg_tensor = torch.zeros((new_first_dim, *old_shape[1:]), device=param_state["exp_avg"].device, dtype=param_state["exp_avg"].dtype)
                comp_avg_sq_tensor = torch.zeros((new_first_dim, *old_shape[1:]), device=param_state["exp_avg_sq"].device, dtype=param_state["exp_avg_sq"].dtype)
            else:
                comp_avg_tensor = torch.zeros_like(param_state["exp_avg"])[:dup_mask.shape[0]].repeat(*repeat_dims)
                comp_avg_sq_tensor = torch.zeros_like(param_state["exp_avg_sq"])[:dup_mask.shape[0]].repeat(*repeat_dims)
            param_state["exp_avg"] = torch.cat([param_state["exp_avg"], comp_avg_tensor], dim=0)
            param_state["exp_avg_sq"] = torch.cat([param_state["exp_avg_sq"], comp_avg_sq_tensor], dim=0)
            del optimizer.state[old_params]
            optimizer.state[new_params[0]] = param_state
            optimizer.param_groups[group_idx]["params"] = new_params
            del old_params

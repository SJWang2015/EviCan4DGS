...
class VanillaGaussians(nn.Module):
    ...
    @torch.no_grad()
    def update_rolling_Ed(self, depths, W=100):
        if (not hasattr(self, "depth_buffer_t")) or (self.depth_buffer_t is None) or (self.depth_buffer_t.numel() != W):
            device = depths.device if torch.is_tensor(depths) else torch.device("cpu")
            self.depth_buffer_t = torch.empty(W, device=device, dtype=torch.float32)
            self.depth_ptr = 0
            self.depth_count = 0

        if depths is None:
            return None if self.depth_count == 0 else self.depth_buffer_t[:self.depth_count].mean()
        if not torch.is_tensor(depths):
            depths = torch.as_tensor(depths, device=self.depth_buffer_t.device, dtype=self.depth_buffer_t.dtype)
        if depths.numel() == 0:
            return None if self.depth_count == 0 else self.depth_buffer_t[:self.depth_count].mean()

        mean_depth = depths.mean().detach()
        self.depth_buffer_t[self.depth_ptr] = mean_depth
        self.depth_ptr = (self.depth_ptr + 1) % W
        self.depth_count = min(self.depth_count + 1, W)

        return self.depth_buffer_t[:self.depth_count].mean()
    ...
    def after_train(
        ...
        with torch.no_grad():
            ...
            if self.xys_grad_norm is None:
                ...
                if self.ctrl_cfg.scale_grad and self.step >= self.ctrl_cfg.warm_scale_grad_steps:
                #     _depth_base = self.update_rolling_Ed(depth, 100) * 0.15
                #     if self.step < 100:
                #         self.xys_grad_norm[full_mask] = grads[visible_mask] + self.xys_grad_norm[full_mask]
                #     else:
                #         depth_base = _depth_base if _depth_base is not None and _depth_base < 5.0 else 5.0
                    depth_weight = depth / (depth + self.ctrl_cfg.depth_mag) # depth_weight = depth /  4.0
                    # depth_weight = depth_weight.clamp_max(0.1) #  depth_weight = depth_weight.clamp_max(1.0)
                    far_mask = depth >= 10.0
                    depth_weight[far_mask] = 1.0
                    # depth_weight = depth /  6.0
                    # depth_weight = depth_weight.clamp_max(1.0)
                    self.xys_grad_norm[full_mask] = grads[visible_mask] + depth_weight[full_mask] * self.xys_grad_norm[full_mask]
                else:
                    self.xys_grad_norm[full_mask] = grads[visible_mask] + self.xys_grad_norm[full_mask]
    ...
    def refinement_after(self, step, optimizer: torch.optim.Optimizer) -> None:
        ...
        with torch.no_grad():
            ...
            # reset opacity
            if "Background" in self.class_prefix:
                    self.keep_opacities = self._opacities.data * 0.5
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict:
        ...
        # with torch.no_grad():
        #     step_times = self.step // self.ctrl_cfg.reset_alpha_interval 
        #     if step_times % 2 > 0 and self.step >self.ctrl_cfg.reset_alpha_interval and self.step < self.ctrl_cfg.refine_alpha_stop_at and "Background" in self.class_prefix:
        #         if self.keep_opacities is None:
        #             self.keep_opacities = self._opacities.data * 0.5
        #         self._opacities.data = self.keep_opacities
    ...
    # def get_gaussians(self, src_pcs, tgt_pcs, cam: dataclass_camera) -> Dict:
    #     filter_mask = torch.ones_like(self._means[:, 0], dtype=torch.bool)
    #     self.filter_mask = filter_mask

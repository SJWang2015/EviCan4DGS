...
class RigidNodes(VanillaGaussians):
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        # init_colors2 = torch.rand((self.num_points, 3), device=self.device)
        # init_colors[self.point_types[:,0]!=1] = init_colors2[self.point_types[:,0]!=1]
    ...
    def refinement_after(self, step: int, optimizer: torch.optim.Optimizer) -> None:
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                if self.ctrl_cfg.scale_grad:
                    avg_grad_norm = self.xys_grad_norm / (self.vis_counts + 1e-6)**self.ctrl_cfg.scale_alpha
                else:
    ...
    def split_gaussians(self, split_mask: torch.Tensor, samps: int = 2) -> Tuple:
        ...
        return new_means, new_feature_dc, new_feature_rest, new_opacities, new_scales, new_quats, new_ids#, new_smpl_masks
    ...
    def dup_gaussians(self, dup_mask: torch.Tensor) -> Tuple:
        ...
        return dup_means, dup_feature_dc, dup_feature_rest, dup_opacities, dup_scales, dup_quats, dup_ids#, dup_smpl_masks
    ...
    def transform_means(self, means: torch.Tensor) -> torch.Tensor:
        ...
        if self.in_test_set and (
            cur_frame - 1 > 0 and cur_frame + 1 < self.num_frames
        ):
            # use the previous and next frame to interpolate the pose
            _quats_prev_frame = self.instances_quats[cur_frame - 1]
            _quats_next_frame = self.instances_quats[cur_frame + 1]
            _quats_cur_frame = self.instances_quats[cur_frame]
            ...
            quats_cur_frame = self.instances_quats[cur_frame] # (num_instances, 4)
        
        rot_cur_frame = None
        if len(quats_cur_frame.shape) == 3:
        ...
        if self.in_test_set and (
            ...
            _prev_ins_trans = self.instances_trans[cur_frame - 1]
            _next_ins_trans = self.instances_trans[cur_frame + 1]
            _cur_ins_trans = self.instances_trans[cur_frame]
            ...
            trans_cur_frame = self.instances_trans[cur_frame] # (num_instances, 3)
    ...
    def transform_quats(self, quats: torch.Tensor) -> torch.Tensor:
        ...
        if self.in_test_set and (
            ...
            inter_valid_mask = self.instances_fv[cur_frame - 1] & self.instances_fv[cur_frame + 1]
            if interpolated_quats.dim() == 3:
                global_quats_cur_frame = torch.where(
            ...
            global_quats_cur_frame = self.instances_quats[cur_frame] # (num_instances, 4)

        if global_quats_cur_frame.dim() == 3:
            global_quats_cur_frame = global_quats_cur_frame[:,0,:] # (num_instances, 4)
        global_quats_per_pts = global_quats_cur_frame[self.point_ids[..., 0]]
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        world_means = self.transform_means(self._meanss)
    ...
    def compute_reg_loss(self) -> Dict[str, torch.Tensor]:
        ...
        #     # assert hasattr(self, "_gs_cache") and "_scales" in self._gs_cache
        #     # assert self._gs_cache["_scales"].grad_fn is not None  # 说明带图
        ...
        if scaling_reg is not None:
            ...
            if self.step < stop_after and self.step > start_after and w > 0:
                # 重算当轮带图的 activated_scales（不要从 cache 拿）
                if self.ball_gaussians:
                    activated_scales = torch.exp(self._scales.repeat(1, 3))
                else:
                    activated_scales = torch.exp(self._scales)

                activated_scales = activated_scales[self.filter_mask]  # 若 filter_mask 是当轮的（更好是局部传入）
                scale_prod = activated_scales.prod(dim=-1)

                # percentile阈值本身不需要梯度，建议 detach，避免不必要的复杂图
                k = max(1, int(scale_prod.shape[0] * precentile))
                p = torch.kthvalue(scale_prod.detach(), k).values
    ...
    def collect_gaussians_from_ids(self, ids: List[int]) -> Dict:
        ...
        for id in ids:
            if id not in gaussian_dict:
                mask = self.point_ids[..., 0] == id 
                # expand_mask = mask.unsqueeze(0).expand(self.u.shape[0], -1).unsqueeze(-1).expand(-1, -1, 3)   # (B, n, 3)
                instance_raw_dict = {

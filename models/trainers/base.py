...
class GSModelType(IntEnum):
    ...
    SMPLNodes = 2
    DeformableNodes = 3
    CPDDeformableNodes = 4
    CPDSMPLDeformableNodes = 5
...
class BasicTrainer(nn.Module):
    ...
    def postprocess_per_train_step(self, step: int) -> None:
        ...
        for class_name in self.gaussian_classes.keys():
            ...
            self.models[class_name].postprocess_per_train_step(
                ...
                depth=self.info["depths"][0, gaussian_mask]
            )
    ...
    def collect_gaussians(
        ...
        # src_pcs: torch.Tensor, # source point clouds, not used now
        # tgt_pcs: torch.Tensor, # source point clouds, not used now
        # processed_cam_list: List[torch.Tensor],
        # human_masks_list: List[torch.Tensor],
        # render_fn = None        
    ) -> dataclass_gs:
        gs_dict = {
            "_means": [],
        ...
        # extras_dicts = None
        # for class_name in self.gaussian_classes.keys():
        #     # src_pcs = image_infos["src_pcs"]
        #     # tgt_pcs = image_infos["tgt_pcs"]
        #     # if class_name == "CPDGaussians":
        #         # CPDGaussians is a special case, it does not have get_gaussians method
        #     if class_name == "CPDSMPLDeformableNodes":
        #         gs, smpl_gs = self.models[class_name].get_gaussians(cam, src_pcs=src_pcs, tgt_pcs=tgt_pcs, processed_cam_list=processed_cam_list, human_masks_list=human_masks_list, render_fn=render_fn)
        #         # extras_dicts = {
        #         #     "cpd_gs": gs,
        #         #     "smpl_gs": smpl_gs, 
        #         # }
        #         continue
        #     else:
        #         gs = self.models[class_name].get_gaussians(cam, src_pcs=src_pcs, tgt_pcs=tgt_pcs, processed_cam_list=processed_cam_list, human_masks_list=human_masks_list)
        #     # gs = self.models[class_name].get_gaussians(cam, src_pcs=src_pcs, tgt_pcs=tgt_pcs)
        #     # else:
        ...
        for class_name in self.gaussian_classes.keys():
            # gs = self.models[class_name].get_gaussians(cam, src_pcs=src_pcs, tgt_pcs=tgt_pcs, processed_cam_list=processed_cam_list, human_masks_list=human_masks_list, render_fn=render_fn)
            gs = self.models[class_name].get_gaussians(cam)
            if gs is None:
                continue
    ...
    #     camera_infos: Dict[str, torch.Tensor],
    #     src_pcs: torch.Tensor,  # source point clouds, not used now
    #     tgt_pcs: torch.Tensor,  # target point clouds, not used now
    #     novel_view: bool = False
    # ) -> Dict[str, torch.Tensor]:
    def forward(
        self, 
        image_infos: Dict[str, torch.Tensor],
        camera_infos: Dict[str, torch.Tensor],
        # src_pcs = None,  # source point clouds, not used now
        # tgt_pcs = None, # target point clouds, not used now
        # cam_id = None,
    ...
    def brightness_weighted_loss(self, pred, gt, alpha=1.0, gamma=2.2, clip_max=None):
        """
        pred, gt: (B, H, W, 3), float in [0,1]
        alpha: 控制亮度权重强度
        gamma: 如果输入是sRGB，可以先做 gamma 校正到线性空间
        clip_max: 最大权重上限，防止过拟合
        """
        gt_lin = torch.pow(torch.clamp(gt, 0.0, 1.0), gamma)#gt #** gamma
        pred_lin = torch.pow(torch.clamp(pred, 0.0, 1.0), gamma)#pred #** gamma


        Y = 0.299*gt_lin[...,0] + 0.587*gt_lin[...,1] + 0.114*gt_lin[...,2]

        weights = 1.0 + alpha * (Y**2)
        if clip_max is not None:
            weights = torch.clamp(weights, max=clip_max)

        mse = ((pred_lin - gt_lin)**2).mean(dim=-1)  # (B,H,W)
    ...
    def compute_losses(
        ...
        if "smpl_rgb" in outputs:
            predicted_smpl_rgb = outputs["smpl_rgb"] * valid_loss_mask[..., None]

            smpl_Ll1 = torch.abs(gt_rgb - predicted_smpl_rgb).mean()
            smpl_simloss = 1 - self.ssim(gt_rgb.permute(2, 0, 1)[None, ...], predicted_smpl_rgb.permute(2, 0, 1)[None, ...])

            loss_dict.update({
                "rgb_loss": self.losses_dict.rgb.w * (Ll1 + smpl_Ll1),
                "ssim_loss": self.losses_dict.ssim.w * (simloss + smpl_simloss),
            })
            # loss_dict.update({
            #     "rgb_loss": self.losses_dict.rgb.w * smpl_Ll1,
            #     "ssim_loss": self.losses_dict.ssim.w * smpl_simloss,
            # })
        ...
        if self.sky_opacity_loss_fn is not None:
            ...
            # if "smpl_opacity" in outputs:
            #     pred_smpl_occupied_mask = outputs["smpl_opacity"].squeeze() * valid_loss_mask
            #     smpl_sky_loss_opacity = self.sky_opacity_loss_fn(pred_smpl_occupied_mask, gt_occupied_mask) * self.losses_dict.mask.w
            #     loss_dict.update({"sky_loss_opacity": sky_loss_opacity + smpl_sky_loss_opacity})
            # else:
        ...
        if self.depth_loss_fn is not None:
            ...
            if "smpl_depth" in outputs:
                pred_smpl_depth = outputs["smpl_depth"]
                smpl_depth_loss = self.depth_loss_fn(pred_smpl_depth, gt_depth, lidar_hit_mask)
                depth_loss = depth_loss + smpl_depth_loss
        ...
        if opacity_entropy_reg is not None:
            ...
            if "smpl_opacity" in outputs:
                pred_smpl_opacity = torch.clamp(outputs["smpl_opacity"].squeeze(), 1e-6, 1 - 1e-6)
                loss_dict.update({
                    "opacity_entropy_loss": opacity_entropy_reg.w * (-pred_opacity * torch.log(pred_opacity) -pred_smpl_opacity * torch.log(pred_smpl_opacity)).mean()
                })
            else:
                loss_dict.update({
        ...
        if inverse_depth_smoothness_reg is not None:
            ...
            if "smpl_depth" in outputs:
                inverse_smpl_depth = 1 / (outputs["smpl_depth"] + 1e-5)
                loss_inv_smpl_depth = kornia.losses.inverse_depth_smoothness_loss(
                    inverse_smpl_depth[None].repeat(1, 1, 1, 3).permute(0, 3, 1, 2),
                    image_infos["pixels"][None].permute(0, 3, 1, 2)
                )
                loss_dict.update({
                    "inverse_depth_smoothness_loss": inverse_depth_smoothness_reg.w * (loss_inv_smpl_depth + loss_inv_depth)
                })
            else:
                loss_dict.update({
        ...
        if dynamic_region_weighted_losses is not None:
            ...
            if self.step > start_from and "Dynamic_opacity" in outputs:
                ...
                smpl_Ll1 = 0.0
                dynamic_pred_smpl_mask = None

                if "smpl_Dynamic_opacity" in outputs:
                    dynamic_pred_smpl_mask = (outputs["smpl_Dynamic_opacity"].data > 0.2).squeeze()
                    dynamic_pred_smpl_mask = dynamic_pred_smpl_mask & valid_loss_mask.bool()
                    if dynamic_pred_smpl_mask.sum() > 0:
                        smpl_Ll1 = torch.abs(
                            gt_rgb[dynamic_pred_smpl_mask] - predicted_smpl_rgb[dynamic_pred_smpl_mask]
                        ).mean()
                ...
                if dynamic_pred_mask.sum() > 0:
                    ...
                    vehicle_region_rgb_loss = weight_factor * (Ll1 + smpl_Ll1)
                elif (dynamic_pred_smpl_mask is not None) and (dynamic_pred_smpl_mask.sum() > 0):
                    vehicle_region_rgb_loss = weight_factor * smpl_Ll1
                else:
                    vehicle_region_rgb_loss = 0.0

                loss_dict["vehicle_region_rgb_loss"] = vehicle_region_rgb_loss
    ...
    def load_state_dict(self, state_dict: dict, load_only_model: bool =True, strict: bool = True):
        ...
        for class_name in self.models.keys():
            ...
            if class_name not in model_state_dict:
                ...
                continue
            # if class_name == "CPDSMPLDeformableNodes":
            #     model_state_dict[class_name]['instances_quats'] = model_state_dict[class_name]['instances_quats'].squeeze(2).contiguous()

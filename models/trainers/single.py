...
from models.gaussians import CPDGaussians
...
class SingleTrainer(BasicTrainer):
        ...
        for class_name in self.gaussian_classes:
            ...
            if class_name == 'Background':                
                ...
                if init_cfg.get("from_lidar", None) is not None:
                    if init_cfg.from_lidar.get('return_flow', False):
                ...
                if num_near_pts > 0: # uniformly sample points inside the scene's sphere
                    num_near_pts *= 1 # since some invisible points will be filtered out
                ...
                elif isinstance(model, CPDGaussians):
                    model.create_from_pcd(
                        init_means=sampled_pts, init_colors=sampled_color, init_times=sampled_time
                    )
                else:
    ...
    def forward(
        ...
        #     camera_infos=camera_infos_list[cam_id],
        #     image_ids=image_infos_list[cam_id]["img_idx"].flatten()[0],

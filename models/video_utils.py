...
def render(
    ...
    Background_rgbs, RigidNodes_rgbs, DeformableNodes_rgbs, CPDDeformableNodes_rgbs, CPDSMPLDeformableNodes_rgbs, SMPLNodes_rgbs, Dynamic_rgbs = [], [], [], [], [], [], []
    error_maps = []

    # depths
    depths, lidar_on_images = [], []
    Background_depths, RigidNodes_depths, DeformableNodes_depths, CPDDeformableNodes_depths, CPDSMPLDeformableNodes_depths, SMPLNodes_depths, Dynamic_depths = [], [], [], [], [], [], []

    # sky
    opacities, sky_masks = [], []
    Background_opacities, RigidNodes_opacities, DeformableNodes_opacities, CPDDeformableNodes_opacities, CPDSMPLDeformableNodes_opacities, SMPLNodes_opacities, Dynamic_opacities = [], [], [], [], [], [], []
    ...
    with torch.no_grad():
        ...
        # src_pcs, src_color, src_flow, src_time = lidar_dataset.get_pcs() if lidar_dataset is not None else None
        # # lidar_infos = lidar_dataset.lidar_source.get_lidar_rays(closest_lidar_idx)
        # tgt_pcs = src_pcs + src_flow
        # image_infos_list = []
        # cam_infos_list = []
        # human_masks_list = []
        # processed_cam_list = []
        ...
        #     # processed_cam = trainer.process_camera(camera_infos=cam_infos, image_ids=image_infos["img_idx"].flatten()[0], novel_view=False)
        #     # processed_cam_list.append(processed_cam)
        #     # human_masks = image_infos['human_masks']
        #     # human_masks_list.append(human_masks)
        #     image_infos_list.append(image_infos)
        #     cam_infos_list.append(cam_infos)

        for i, item in enumerate(tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True)):
            image_infos, cam_infos = dataset.get_image(item, camera_downscale)
            for k, v in image_infos.items():
                if isinstance(v, Tensor):
                    image_infos[k] = v.cuda(non_blocking=True)
            for k, v in cam_infos.items():
                if isinstance(v, Tensor):
                    cam_infos[k] = v.cuda(non_blocking=True)
            src_pcs = image_infos["lidar_points_map"]
            # src_flow = image_infos_list[i]["lidar_flows_map"]
            # flow_classes = image_infos_list[i]["lidar_flow_classes_map"]
            # if local_pts:
            #     frame_idx = image_infos_list[i]['frame_idx'][0][0]
            #     cur_l2w = lidar_dataset.lidar_source.lidar_to_worlds[frame_idx,:,:]
            #     cur_l2w_inv = torch.linalg.inv(cur_l2w)
            #     src_pcs = (cur_l2w_inv[:3,:3].T @ src_pcs.T).T + cur_l2w[:3,-1]
            #     src_flow = (cur_l2w_inv[:3,:3].T @ src_flow.T).T + cur_l2w[:3,-1]
            # tgt_pcs = src_pcs + src_flow
            # render the image
            # results = trainer(image_infos=image_infos_list[i], camera_infos=cam_infos_list[i], cam_id=i, processed_cam_list=processed_cam_list, human_masks_list=human_masks_list, src_pcs=src_pcs, tgt_pcs=tgt_pcs,  novel_view=False)
            results = trainer(image_infos=image_infos, camera_infos=cam_infos, novel_view=False)
            # ------------- clip rgb ------------- #
            ...
            if "CPDDeformableNodes_rgb" in results:
                CPDDeformableNodes_rgb = results["CPDDeformableNodes_rgb"] * results[
                    "CPDDeformableNodes_opacity"
                ] + green_background * (1 - results["CPDDeformableNodes_opacity"])
                CPDDeformableNodes_rgbs.append(get_numpy(CPDDeformableNodes_rgb))
            if "CPDSMPLDeformableNodes_rgb" in results:
                CPDSMPLDeformableNodes_rgb = results["CPDSMPLDeformableNodes_rgb"] * results[
                    "CPDSMPLDeformableNodes_opacity"
                ] + green_background * (1 - results["CPDSMPLDeformableNodes_opacity"])
                CPDSMPLDeformableNodes_rgbs.append(get_numpy(CPDSMPLDeformableNodes_rgb))
            if "smpl_rgb" in results:
                SMPLNodes_rgb = results["smpl_rgb"]
            ...
            if "CPDDeformableNodes_depth" in results:
                CPDDeformableNodes_depths.append(get_numpy(results["CPDDeformableNodes_depth"]))
                CPDDeformableNodes_opacities.append(get_numpy(results["CPDDeformableNodes_opacity"]))
            if "CPDSMPLDeformableNodes_depth" in results:
                CPDSMPLDeformableNodes_depths.append(get_numpy(results["CPDSMPLDeformableNodes_depth"]))
                CPDSMPLDeformableNodes_opacities.append(get_numpy(results["CPDSMPLDeformableNodes_opacity"]))
            ...
            if compute_metrics:
                ...
                logger.info(f"Frame {item}: PSNR {psnr:.4f}, SSIM {ssim_score:.4f}")
                psnrs.append(psnr)
    ...
    if len(CPDDeformableNodes_rgbs) > 0:
        results_dict["CPDDeformableNodes_rgbs"] = CPDDeformableNodes_rgbs
    if len(CPDSMPLDeformableNodes_rgbs) > 0:
        results_dict["CPDSMPLDeformableNodes_rgbs"] = CPDSMPLDeformableNodes_rgbs
    ...
    if len(CPDDeformableNodes_depths) > 0:
        results_dict["CPDDeformableNodes_depths"] = CPDDeformableNodes_depths
    if len(CPDSMPLDeformableNodes_depths) > 0:
        results_dict["CPDSMPLDeformableNodes_depths"] = CPDSMPLDeformableNodes_depths
    ...
    if len(CPDDeformableNodes_opacities) > 0:
        results_dict["CPDDeformableNodes_opacities"] = CPDDeformableNodes_opacities
    if len(CPDSMPLDeformableNodes_opacities) > 0:
        results_dict["CPDSMPLDeformableNodes_opacities"] = CPDSMPLDeformableNodes_opacities
...
def render_novel_views(trainer, render_data: list, save_path: str, fps: int = 30, lidar_dataset: DrivingDataset = None, local_pts: bool=False) -> None:
    """
    ...
    with torch.no_grad():
        for frame_data in render_data:
            ...
            src_pcs = frame_data["image_infos"]["lidar_points_map"]
            src_flow = frame_data["image_infos"]["lidar_flows_map"] 
            # flow_classes = frame_data["image_infos"]["lidar_flow_classes_map"]
    
            if local_pts:
                frame_idx = frame_data["image_infos"]['frame_idx'][0][0]
                cur_l2w = lidar_dataset.lidar_source.lidar_to_worlds[frame_idx,:,:]
                cur_l2w_inv = torch.linalg.inv(cur_l2w)
                src_pcs = (cur_l2w_inv[:3,:3].T @ src_pcs.T).T + cur_l2w[:3,-1]
                src_flow = (cur_l2w_inv[:3,:3].T @ src_flow.T).T + cur_l2w[:3,-1]
            ...
            # Perform rendering
            outputs = trainer(
                processed_cam_list=None, human_masks_list=None, cam_id=None, 

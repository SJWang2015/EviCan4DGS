...
# 获取项目根目录的绝对路径
PYBCPD_ROOT = os.path.abspath(os.path.dirname(__file__))

# 获取C源文件列表
def get_c_sources():
    c_sources = [
        os.path.join('src', 'pybcpd', 'register', 'bcpd_wrapper.c'),
        os.path.join('src', 'pybcpd', 'register', 'bcpd.c'),
        os.path.join('src', 'pybcpd', 'register', 'info.c'),
        os.path.join('src', 'pybcpd', 'register', 'norm.c'),
    ]
    
    # base文件夹中的源文件
    base_dir = os.path.join('src', 'pybcpd', 'base')
    base_files = [
        'util.c', 'misc.c', 'kdtree.c', 'kernel.c', 'gaussprod.c', 'dijkstra.c',
        'sampling.c', 'sgraph.c', 'geokdecomp.c', 'mt19937-64.c', 'median.c', 
        'digamma.c', 'heap.c', 'gramdecomp.c'
    ]
    
    c_sources.extend([os.path.join(base_dir, f) for f in base_files])
    return c_sources
...
def get_extra_compile_args():
    ...
    if platform.system() == 'Windows':
        ...
        args = ['-O3', '-fPIC', '-std=c99']
    
    # OpenMP支持
    if platform.system() == 'Darwin':  # macOS
        if platform.machine() == 'arm64':  # M1/M2 Mac
            args.extend(['-Xpreprocessor', '-fopenmp', '-I/opt/homebrew/opt/libomp/include/'])
        else:  # Intel Mac
            args.extend(['-Xpreprocessor', '-fopenmp', '-I/usr/local/include/'])
    elif platform.system() != 'Windows':  # Linux
        args.extend(['-fopenmp', "-DUSE_OPENMP"])
...
def get_extra_link_args():
    ...
    if platform.system() == 'Windows':
        ...
        if platform.machine() == 'arm64':  # M1/M2 Mac
            return ['/opt/homebrew/opt/libomp/lib/libomp.dylib', '-llapack']
        else:  # Intel Mac
            return ['/usr/local/lib/libomp.dylib', '-llapack']
    else:  # Linux
        return ['-lm', '-llapack', '-fopenmp']

# 获取所有需要的包含目录
def get_include_dirs():
    """获取所有需要的包含目录"""
    include_dirs = [
        os.path.join(PYBCPD_ROOT, 'src', 'pybcpd'),
        os.path.join(PYBCPD_ROOT, 'src', 'pybcpd', 'base'),
        os.path.join(PYBCPD_ROOT, 'src', 'pybcpd', 'register'),
        np.get_include()
    ]
...
# 创建C API包装文件
def create_c_api_file():
    """创建C API包装文件"""
    c_api_path = os.path.join('src', 'pybcpd', 'register', 'bcpd_c_api.c')
    
    # 检查文件是否已存在
    if os.path.exists(c_api_path):
        print(f"C API包装文件已存在: {c_api_path}")
        return c_api_path
    
    print(f"创建C API包装文件: {c_api_path}")
    
    # 创建包装文件目录
    os.makedirs(os.path.dirname(c_api_path), exist_ok=True)
    
    # 这里放入bcpd_c_api.c的内容(前面提供的代码)
    with open(c_api_path, 'w') as f:
        f.write("""/* bcpd_c_api.c - Python C API 包装 BCPD 算法 */
...
bcpd_register(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!O!|idddddiipiiiiiidiOdddpiiddi:bcpd_register", kwlist,
...
# 自定义build_ext命令以处理编译选项
class BuildExt(build_ext):
    """自定义build_ext以设置编译器标志"""
    ...
    def build_extensions(self):
        ...
        if platform.system() == "Darwin":
            print(f"macOS架构: {platform.machine()}")
        ...
        for ext in self.extensions:
            ...
            for source in ext.sources:
                print(f"    - {source}")
            print(f"  编译选项: {ext.extra_compile_args}")
            print(f"  链接选项: {ext.extra_link_args}")
            print(f"  包含目录:")
...
extensions = [
    ...
    # 纯C API的BCPD扩展
    Extension(
        'pybcpd._bcpd',  # 使用与原pybind11模块相同的名称
        sources=[c_api_path] + get_c_sources(),
        include_dirs=get_include_dirs(),
        extra_compile_args=get_extra_compile_args(),
        extra_link_args=get_extra_link_args(),
        libraries=["lapack", "blas", 'm'],
...
setup(
    name="pybcpd",
    version="0.1.0",
    author="BCPD Python Interface Developer",
    author_email="your.email@example.com",
    description="Python interface for Bayesian Coherent Point Drift",
    long_description="A Python interface for BCPD (Bayesian Coherent Point Drift) algorithm for point cloud registration",

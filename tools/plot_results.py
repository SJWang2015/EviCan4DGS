...
def main():
    ...
    # parser.add_argument('--plot_human', default=True, action='store_true')
    ...
    results_pattern = r"Frame\s+(?P<frame>\d+):\s+PSNR\s+(?P<psnr>\d+(?:\.\d+)?),\s+SSIM\s+(?P<ssim>\d+(?:\.\d+)?)"
    ...
    for i, log_file in enumerate(args.log_files):
        ...
        for r in re.findall(results_pattern, log):
        ...
        max_episodes = train_episode[-1]

        for r in re.findall(results_pattern, log):
            # if r[0] == "PSNR":
            train_psnr.append(float(r[1]))
            # if r[0] == "SSIM":
            train_ssim.append(float(r[2]))
            # if r[0] == "LPIPS":

...
def setup(args):
    ...
    log_dir = os.path.join(args.output_root, args.project, args.run_name, str(cfg['data']["scene_idx"]))
...
def main(args):
    ...
    # aa = np.load("/home/ubuntu/Repositories/drivestudio/0.npy")
    # build dataset
    ...
    local_pts = False
    if cfg.trainer.type == "models.trainers.MultiTrainer":
        local_pts = True
    ...
    #     args=args,
    # )
    # for step in tqdm(all_iters, desc="Training", total=len(all_iters)):
    for step in metric_logger.log_every(all_iters, cfg.logging.print_freq):
        ...
        # src_flow = image_infos["lidar_flows_map"] 
        ...
        outputs = trainer(image_infos, cam_infos, src_pcs=src_pcs, novel_view=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--entity", default="ubuntu", type=str, help="wandb entity name")

...
root = "logs/cpddeform_waymo/nvs"
root = "logs/omnire_waymo/nvs"
split = "test"
# scene reconstruction results
# root = "logs/cpddeform_waymo/recon"
# split = "full"
...
for scene_id in scene_ids:
    try:
        metric = os.listdir(os.path.join(root, str(scene_id), str(scene_id),"metrics"))
        metric = [m for m in metric if m.startswith(f"images_{split}")][0]
        with open(os.path.join(root, str(scene_id), str(scene_id), "metrics", metric), "r") as f:
            data = json.load(f)
    except:

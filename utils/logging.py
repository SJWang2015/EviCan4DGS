...
class MetricLogger(object):
    ...
    def log_every(
        ...
        iter_time = self.meters["iter_time"] = SmoothedValue(fmt="{avg:.6f}")
        data_time = self.meters["data_time"] = SmoothedValue(fmt="{avg:.6f}")
        ...
        # 使用 tqdm 包装 iterable
        pbar = tqdm(iterable, desc=header or "Progress", total=n_iterations, initial=i)
        ...
        for obj in pbar:
            ...
            if i % print_freq == 0 or i == n_iterations - 1:
                ...
                logger.info(log_str)
                # 更新 tqdm 的 postfix 内容（右侧显示）
                # pbar.set_postfix_str(str(self))
            pbar.set_postfix({"psnr": self.meters["train_metrics/psnr"].global_avg,
                                  "ssim_loss": self.meters["losses/ssim_loss"].global_avg,
                                  "rgb_loss": self.meters["losses/rgb_loss"].global_avg})

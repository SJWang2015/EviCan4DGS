...
class SurfaceReconstruction:
    ...
    def reconstruct_surface(self, 
                        #   points: Union[torch.Tensor, np.ndarray], 
                          ...
                          param: float = 0.1,
                          param2: float = 0.1) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        从3D点云重建表面mesh
        ...
        if self.is_cuda_available():
            return self._cuda_reconstruct( X, Y, Z, param, param2)
        else:
            return self._cpu_reconstruct( X, Y, Z, param)
    
    def _cuda_reconstruct(self, X: torch.Tensor, Y: torch.Tensor, Z: torch.Tensor, param: float, param2: float) -> Tuple[torch.Tensor, torch.Tensor]:
        """使用CUDA进行表面重建"""
        try:
            # 确保数据在GPU上
            if not X.is_cuda:
            ...
            #         X, Y, Z, param  
            #     )
            if hasattr(self.cuda_module, 'surface_reconstruction') and hasattr(self.cuda_module, "find_four_grid_neighbors") and hasattr(self.cuda_module, "calculate_normals_from_grid"):
                four_nn_inds = self.cuda_module.find_four_grid_neighbors(X, Y, Z)
                normals = self.cuda_module.calculate_normals_from_grid(X, Y, Z, four_nn_inds)
                triangles_result = self.cuda_module.surface_reconstruction(X, Y, Z, four_nn_inds, normals, param, param2)
            elif hasattr(self.cuda_module, 'surface_reconstruction_cuda'):
                # 使用完整版本 - 需要转换数据格式
                triangles_result = self._call_full_cuda_function(X, Y, Z, algorithm_type, param)
            else:
